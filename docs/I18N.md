...
├── app/
│   ├── i18n.py                    # i18n utilities and gettext functions
│   ├── main.py                    # Middleware and Jinja2 configuration
│   └── schemas.py                 # Translatable validation messages
├── translations/                   # Translation files directory
│   └── pt_BR/                     # Portuguese (Brazil) translations
│       └── LC_MESSAGES/
│           ├── messages.po        # Editable translation source
...
<p>{{ _('Thanks %(name)s — your message has been received!', name=contact.name) }}</p>
...
- ✅ Automatic convenience for most users
...
- **Chrome**: Settings → Languages → Add language → Move to top
...
✅ Buttons (Send Message, Reset, Delete)
✅ Validation errors (server and client-side)
✅ Success messages
✅ Admin interface
✅ Login page
✅ Navigation elements
...
1. ✅ Extracts all translatable messages
2. ✅ Updates translation files
3. ✅ Compiles translations (fails if .po files have errors)
4. ✅ Reports translation completeness statistics
...
- ✅ Works without babel in production
- ✅ Ensures exact same translations in all environments
- ✅ CI validates them before merge
